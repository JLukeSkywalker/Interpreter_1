...
extern "C" {
    ...
    static int hashmap_create(const unsigned initial_size,
        struct hashmap* const out_hashmap) HASHMAP_USED;
    ...
    static int hashmap_put(struct hashmap* const hashmap, const char* const key,
        const unsigned len, struct Data const value) HASHMAP_USED;
    ...
    static struct Data hashmap_get(const struct hashmap* const hashmap,
    ...
    /// @param len The length of the string key.
    /// @return On success 0 is returned.
    static int hashmap_remove(struct hashmap* const hashmap,
        const char* const key,
        const unsigned len) HASHMAP_USED;
    ...
    static int hashmap_iterate_pairs(struct hashmap* const hashmap,
    ...
    static unsigned
        hashmap_num_entries(const struct hashmap* const hashmap) HASHMAP_USED;

    /// @brief Destroy the hashmap.
    /// @param hashmap The hashmap to destroy.
    static void hashmap_destroy(struct hashmap* const hashmap) HASHMAP_USED;

    static unsigned hashmap_crc32_helper(const char* const s,
        const unsigned len) HASHMAP_USED;
    static unsigned
        hashmap_hash_helper_int_helper(const struct hashmap* const m,
            const char* const keystring,
            const unsigned len) HASHMAP_USED;
    static int hashmap_match_helper(const struct hashmap_element* const element,
        const char* const key,
        const unsigned len) HASHMAP_USED;
    static int hashmap_hash_helper(const struct hashmap* const m,
    ...
    static int hashmap_rehash_iterator(void* const new_hash,
        struct hashmap_element* const e) HASHMAP_USED;
    static int hashmap_rehash_helper(struct hashmap* const m) HASHMAP_USED;
...
int hashmap_put(struct hashmap* const m, const char* const key,
    const unsigned len, struct Data const value) {
    unsigned int index;
...
struct Data hashmap_get(const struct hashmap* const m, const char* const key,
    const unsigned len) {
    unsigned int curr;
    unsigned int i;
...
int hashmap_remove(struct hashmap* const m, const char* const key,
    const unsigned len) {
    unsigned int i;
    unsigned int curr;
...
int hashmap_iterate_pairs(struct hashmap* const hashmap,
    int (*f)(void* const, struct hashmap_element* const),
    void* const context) {
    unsigned int i;
...
void hashmap_destroy(struct hashmap* const m) {
    free(m->data);
    memset(m, 0, sizeof(struct hashmap));
}

unsigned hashmap_num_entries(const struct hashmap* const m) {
    return m->size;
}
...
unsigned hashmap_hash_helper_int_helper(const struct hashmap* const m,
    const char* const keystring,
    const unsigned len) {
...
int hashmap_match_helper(const struct hashmap_element* const element,
    const char* const key, const unsigned len) {
    return (element->key_len == len) && (0 == memcmp(element->key, key, len));
}

int hashmap_hash_helper(const struct hashmap* const m, const char* const key,
...
int hashmap_rehash_iterator(void* const new_hash,
    ...
    int temp = hashmap_put(HASHMAP_PTR_CAST(struct hashmap*, new_hash),
